...
class VirtualFileSystem:
    ...
    @contextmanager
    @abstractmethod
    def open(self, filepath: str, mode: str = "r", **kwargs) -> VirtualFile:
        """打开文件"""
        raise NotImplementedError()
    ...
    @abstractmethod
    def list_files(self, dirpath: str, **kwargs) -> list[str]:
        """列出目录下的文件"""
        raise NotImplementedError()
...
class MemoryFileSystem(VirtualFileSystem):
    ...
    def load(self, dirpath: str, overwrite: bool = False):
        """从磁盘中加载文件
        ...
        if not dirpath.startswith(self._root_dir):
            raise ValueError(f"Invalid directory: '{dirpath}': not in root directory '{self._root_dir}'")
        for root, dirs, files in os.walk(dirpath):
            for file in files:
                disk_filepath = os.path.join(root, file)
                with open(disk_filepath, "r") as f:
                    content = f.read()
                filepath = self._normalize_path(os.path.relpath(disk_filepath, self._root_dir))
                if overwrite or not self.exists(filepath):
                    self._files[filepath] = MemoryFile(content, "r")

    def _normalize_path(self, path: str) -> str:
        if not path.startswith("/"):
            filepath = "/" + os.path.join(self._root_dir.strip("/"), path)
        else:
            filepath = path
    ...
    def flush(self):
        for filepath, file in self._files.items():
            if file.modified:
                try:
                    disk_filepath = filepath.lstrip("/")
                    os.makedirs(os.path.dirname(disk_filepath), exist_ok=True)
                    with open(disk_filepath, "w") as f:
                        f.write(file.content)
                    file._modified = False
                except Exception as e:
                    print("Flush error:", e)
